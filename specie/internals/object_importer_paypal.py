...
class ObjPayPalImporter(ObjImporter, typename = "PayPalImporter"):
  ...
  def parse_record(self, id, record, source):
    ...
    amount = ObjMoney(ObjString(record['Valuta']), ObjFloat(record['Net'].replace(',','.')))
    from_email = record['Van e-mailadres'].lower()
    to_email = record['Naar e-mailadres'].lower()
    own_address = from_email if amount['value'] < ObjFloat(0.0) else to_email
    address = to_email if amount['value'] < ObjFloat(0.0) else from_email

    return ObjTransaction(
      # Standard fields
      id = ObjString(f"paypal:{source}:{id:06d}:{record['Transactiereferentie']}"),
      date = ObjDate(timestamp),
      amount = amount,
      name = ObjString(record['Naam'].upper()),
      address = ObjString(address),
      description = ObjString(record['Item Title'] or record['Note']),

      # Extension fields
      source = Field(ObjString(source), public = False),
      own_address = Field(ObjString(own_address), public = False),
      type = Field(ObjString(record['Type']), public = False),
      timestamp = Field(ObjFloat(timestamp.timestamp()), public = False))
