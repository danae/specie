...
class ObjTransaction(ObjRecord):
  ...
  def __init__(self, **kwargs):
    ...
    self['id'] = (ObjString(), False, False)
    self['date'] = ObjDate()
    self['amount'] = (ObjMoney(currency = ObjString('EUR'), amount = ObjFloat()), True, True, ObjRecordFieldOptions.FORMAT_RIGHT)
    self['label'] = ObjString()
    self['name'] = ObjString()
    self['address'] = ObjString()
    self['description'] = (ObjString(), True, True, ObjRecordFieldOptions.FORMAT_ELLIPSIS)
